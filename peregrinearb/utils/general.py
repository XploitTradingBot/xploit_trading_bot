...
def print_profit_opportunity_for_path(graph, path, round_to=32, depth=False, starting_amount=100):
    if not path:
        return
    ...
    for i in range(len(path) - 1):
        ...
        # num = math.exp(-graph[start][end]['weight'])
        # print(num, round_to)
        ...
        print(printed_line)
        line.append({"{}/{}".format(start, end): {'price': rate, 'amount': resulting_amount, 'info': printed_line}})
    return line
