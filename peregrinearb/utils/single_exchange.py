...
async def load_exchange_graph(exchange, name=True, fees=True, suppress=None, depth=False, tickers=None) -> nx.DiGraph:
    ...
    for i in range(20):
        try:
            ...
            active_currencies = [curr for curr in exchange.markets if exchange.markets[curr]['active'] is True]
    ...
    if tickers is None:
        ...
        tickers = await exchange.fetch_tickers(active_currencies)
    ...
    if fees:
        ...
        currency_count = len(active_currencies)
    ...
    try:
        ...
        if depth:
            ...
            if ask_volume is None:
                ...
                return
        if ":" in market_name:
            adapter.warning('Market is not available because it has ":" in its name', market=market_name)
            return
